  ...
  return (
    <div className="mx-auto max-w-2xl space-y-8">
      ...
      <div className="rounded-xl p-6 shadow-sm" style={{ backgroundColor: "#fff" }}>
        <h2 className="mb-4 text-lg font-semibold">Horaires d'ouverture</h2>
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="mb-1 block text-xs font-medium opacity-70">Matin — Début</label>
          ...
          <div>
            <label className="mb-1 block text-xs font-medium opacity-70">Matin — Fin</label>
          ...
          <div>
            <label className="mb-1 block text-xs font-medium opacity-70">Après-midi — Début</label>
          ...
          <div>
            <label className="mb-1 block text-xs font-medium opacity-70">Après-midi — Fin</label>
          ...
          <div>
            <label className="mb-1 block text-xs font-medium opacity-70">Durée du créneau (min)</label>
      ...
      <div className="rounded-xl p-6 shadow-sm" style={{ backgroundColor: "#fff" }}>
        <h2 className="mb-4 text-lg font-semibold">Jours fériés / Fermetures</h2>
        <div className="mb-4 flex flex-wrap items-end gap-3">
          ...
          <div>
            <label className="mb-1 block text-xs font-medium opacity-70">Libellé (optionnel)</label>
            <input type="text" value={newHolidayLabel} onChange={(e) => setNewHolidayLabel(e.target.value)} placeholder="Ex: Aïd" className="rounded-lg border p-2 text-sm outline-none" style={{ borderColor: "#d1d5db" }} />
          </div>
        ...
        {holidays.length === 0 ? (
          <p className="text-sm opacity-60">Aucun jour férié enregistré.</p>
        ) : (
          <ul className="space-y-2">
            {holidays.map((h) => (
              <li key={h.id} className="flex items-center justify-between rounded-lg bg-gray-50 px-3 py-2 text-sm">
                ...
                <button onClick={() => deleteHoliday(h.id)} className="text-xs text-red-500 hover:underline">Supprimer</button>
              </li>
            ))}
          </ul>
        )}
